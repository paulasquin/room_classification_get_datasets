...
DATASET_FOLDER = "../Datasets/JPG_Scannet_0.2"
...
def delBlankImage(lesImgPath):
    ...
    lesTri = locate_files(extension=".jpg", path = '/media/nas/ScanNet/JPG_Scannet_Matterport_Tri', dbName="matterport_tri")
    lesGood = []
    for tri in lesTri:
        if "-0.5" in tri:
            lesGood.append(tri.replace("-0.5", "-0.2").split("/")[-1])

    for path in lesImgPath:
        if os.path.getsize(path) < 6000 or "-0.2.jpg" not in path:# or path.split("/")[-1] not in lesGood:
